...
async function upsertDemoUser(pool, userSpec, summary) {
  ...
  const canLinkChildren = userSpec.role === "PARENT";
  ...
  await pool.query(
    `
      ...
      SET
        ...
        last_name = $8,
        can_link_children = $9,
        accepted_legal_terms_at = COALESCE(accepted_legal_terms_at, $10::timestamptz)
      WHERE id = $1
    `,
    [
      id,
...
async function ensureResults(pool, studentId, moduleId, summary) {
    ...
    await pool.query(
      `
        ...
        SET
          ...
          module_id = EXCLUDED.module_id
      `,
      [seed.id, studentId, seed.subject, seed.score, seed.outOf, seed.date, moduleId]
    );
...
async function ensureUploads(pool, usersByKey, moduleId, summary) {
  ...
  for (const seed of UPLOAD_SEEDS) {
    ...
    await pool.query(
      `
        INSERT INTO uploads
          (id, kind, original_name, mime_type, size_bytes, storage_path, uploaded_by, module_id, created_at)
        VALUES
          ($1, $2, $3, 'text/plain', $4, $5, $6, $7, now())
        ON CONFLICT (id) DO UPDATE
        SET
          kind = EXCLUDED.kind,
          ...
          module_id = EXCLUDED.module_id
      `,
      [seed.id, seed.kind, seed.originalName, sizeBytes, storagePath, uploadedBy, moduleId]
    );
...
async function run() {
  ...
  try {
    ...
    await ensureThreadWithMessages(
      pgRepos,
      {
        starterId: usersByKey.financeAdmin.id,
        participantEmail: usersByKey.parent.email,
        messages: [
          { authorId: usersByKey.financeAdmin.id, body: "DEMO SEED: Finance admin payment-plan note." },
          { authorId: usersByKey.parent.id, body: "DEMO SEED: Parent acknowledgement for finance office." },
        ],
      },
      summary
    );
    ...
    await ensureThreadWithMessages(
      pgRepos,
      {
        starterId: usersByKey.superAdmin.id,
        participantEmail: usersByKey.admin.email,
        messages: [
          { authorId: usersByKey.superAdmin.id, body: "DEMO SEED: Super admin operations check-in." },
          { authorId: usersByKey.admin.id, body: "DEMO SEED: Academic admin status reply." },
        ],
      },
      summary
    );

    const academicSeed = await ensureAttendance(pool, usersByKey, summary);
    await ensureResults(pool, usersByKey.student1.id, academicSeed.moduleId, summary);
    ...
    await ensureUploads(pool, usersByKey, academicSeed.moduleId, summary);
    ...
    console.log(`ACADEMIC ADMIN ${DEMO_USERS.admin.email} / ${DEMO_USERS.admin.password}`);
    console.log(`FINANCE ADMIN  ${DEMO_USERS.financeAdmin.email} / ${DEMO_USERS.financeAdmin.password}`);
    console.log(`SUPER ADMIN    ${DEMO_USERS.superAdmin.email} / ${DEMO_USERS.superAdmin.password}`);

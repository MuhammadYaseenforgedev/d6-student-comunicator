...
import { adminScopeLabel, isSuperAdmin } from "../lib/adminAccess";
import {
...
export default function AdminUsers() {
  ...
  const canManageAdminAccess = isSuperAdmin(currentUser);
    ...
    () =>
      canManageAdminAccess
        ...
        : ACCOUNT_TYPE_OPTIONS.filter(
            (option) => !ADMIN_ACCOUNT_TYPES.includes(option.value)
          ),
    [canManageAdminAccess]
  );
  ...
  useEffect(() => {
    if (!canManageAdminAccess && ADMIN_ACCOUNT_TYPES.includes(newAccountType)) {
      setNewAccountType("STUDENT");
    }
  }, [canManageAdminAccess, newAccountType]);
  ...
  async function onCreateAccount(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (nextRole === "ADMIN" && !canManageAdminAccess) {
  ...
  async function onSave(account: AdminAccount) {
    ...
    if (
      account.role === "ADMIN" &&
      canManageAdminAccess &&
      editAdminScope !== (account.adminScope ?? "SUPER")
    ) {
  ...
  return (
    <div className="space-y-6">
      ...
      <LearnerCsvImportPanel onImported={loadAccounts} />

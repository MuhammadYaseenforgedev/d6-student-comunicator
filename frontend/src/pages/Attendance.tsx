...
type SessionVisualStatus = "upcoming" | "live" | "closed";

function getSessionVisualStatus(session: AttendanceSession): SessionVisualStatus {
  const now = Date.now();
  const start = session.startsAt ? Date.parse(session.startsAt) : Date.parse(`${session.date}T00:00:00`);
  const end = session.endsAt ? Date.parse(session.endsAt) : Date.parse(`${session.date}T23:59:59`);
...
function sessionStatusClass(status: SessionVisualStatus): string {
  if (status === "live") {
    return "border-[rgba(52,211,153,0.30)] bg-[rgba(52,211,153,0.14)] text-[#d9fff1]";
  }
  if (status === "upcoming") {
    return "border-[rgba(140,235,255,0.24)] bg-[rgba(140,235,255,0.10)] text-[#dffbff]";
  }
  return "border-[rgba(255,196,87,0.24)] bg-[rgba(255,196,87,0.10)] text-[#ffecc2]";
}

function sessionJoinBlockedReason(session: AttendanceSession): string | null {
  if (session.checkedInAt) return "You have already joined this session.";
  const status = getSessionVisualStatus(session);
  if (status === "upcoming") return "This session has not opened yet.";
          ...
          {!selectedModule ? (
            ...
            sessions.map((session) => (
              <div
                key={session.id}
                className={[
                  "rounded-3xl border p-4 transition-all duration-200 hover:-translate-y-[1px] hover:border-[rgba(140,235,255,0.34)] hover:bg-[rgba(14,42,99,0.62)] hover:shadow-[0_0_18px_rgba(140,235,255,0.10)]",
                  getSessionVisualStatus(session) === "live"
                    ? "border-[rgba(52,211,153,0.28)] bg-[rgba(13,72,52,0.34)]"
                    : "border-[rgba(140,235,255,0.18)] bg-[rgba(8,18,48,0.66)]",
                ].join(" ")}
              >
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                  <div>
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      <span
                        className={[
                          ...
                          sessionStatusClass(getSessionVisualStatus(session)),
                        ].join(" ")}
                      >
                    ...
                    <div className="mt-1 text-xs text-white/70">
                      {session.checkedInAt
                        ...
                        : (sessionJoinBlockedReason(session) ?? "Live now. Join to record your check-in time.")}
                    </div>
                  </div>
                  ...
                  <button
                    ...
                    disabled={
                      busySessionId === session.id || Boolean(sessionJoinBlockedReason(session))
                    }
                    className="btn-primary w-full px-4 py-2 text-sm disabled:opacity-60 sm:w-auto"
                  >
                    {session.checkedInAt

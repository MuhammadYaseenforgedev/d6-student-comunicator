...
export default function Faculty() {
  ...
  return (
    <div>
      <PageHeader
        title="Faculty"
        subtitle={`Faculty-wide announcements and notices. (${mode})`}
        actions={actions}
      />
      ...
      <NewAnnouncementModal
        key={`faculty-${open ? "open" : "closed"}`}
        open={open}
        onClose={() => setOpen(false)}
        defaultChannel="faculty"
        onCreate={create}
      />
    </div>
  );
